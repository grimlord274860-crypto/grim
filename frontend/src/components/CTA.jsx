  ...
  return (
    <section className="bg-[#0a0b0f] py-20 lg:py-24 border-t border-white/5">
      <div className="max-w-3xl mx-auto px-6">
        <h2 className="text-white font-bold text-center text-3xl sm:text-4xl mb-10">
        ...
        <div className="space-y-3">
          {faqs.map((f, i) => (
            <div
              key={i}
              className="bg-[#13151b] border border-white/10 rounded-lg overflow-hidden"
            >
              <button
                onClick={() => setOpen(open === i ? -1 : i)}
                className="w-full flex items-center justify-between text-left px-6 py-5 text-white font-semibold hover:bg-white/5 transition-colors"
              >
                <span>{f.question}</span>
                <ChevronDown
                  className={`w-5 h-5 flex-shrink-0 ml-4 transition-transform ${
                    open === i ? 'rotate-180 text-[#00d34d]' : 'text-white/60'
                  }`}
                />
              </button>
              <div
    ...
    <>
      ...
      <section className="bg-[#0a0b0f] py-24 border-t border-white/5 relative overflow-hidden">
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse 50% 60% at 50% 50%, rgba(0,211,77,0.12), transparent 70%)',
          }}
        />
        <div className="relative max-w-3xl mx-auto px-6 text-center">
          ...
          <p className="text-white/70 text-lg mt-5">
            Click the button below to get lifetime access to GainzAlgo while it's still on sale!
          </p>
