    ...
    <section className="bg-[#0a0b0f] py-20 lg:py-28 border-t border-white/5">
      <div className="max-w-[1400px] mx-auto px-6">
        <h2 className="text-white font-bold text-center text-3xl sm:text-4xl lg:text-5xl mb-14">
          Profits made by traders using GainzAlgo
        </h2>
        ...
        <div className="relative">
                ...
                {profitImages
                  ...
                  .map((src, i) => (
                    ...
                    >
                      <img
                        src={src}
                        alt={`Trader profit ${pIdx * visiblePerPage + i + 1}`}
                        loading="lazy"
          ...
          <button
            onClick={handlePrev}
            aria-label="Previous"
            className="absolute -left-2 lg:-left-6 top-1/2 -translate-y-1/2 w-11 h-11 bg-[#13151b] hover:bg-[#00d34d] hover:text-black border border-white/20 text-white rounded-full flex items-center justify-center transition-colors"
          >
          ...
          <button
            onClick={handleNext}
            aria-label="Next"
            className="absolute -right-2 lg:-right-6 top-1/2 -translate-y-1/2 w-11 h-11 bg-[#13151b] hover:bg-[#00d34d] hover:text-black border border-white/20 text-white rounded-full flex items-center justify-center transition-colors"
          >
          ...
          {Array.from({ length: totalPages }).map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${
                i === page ? 'w-8 bg-[#00d34d]' : 'w-2 bg-white/25 hover:bg-white/50'
              }`}
            />
          ))}
        </div>
